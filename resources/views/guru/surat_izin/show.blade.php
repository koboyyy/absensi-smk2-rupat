...
@section ('content')
    ...
    <div class="max-w-4xl space-y-6">
        ...
        >
            <!-- Header Info -->
            <div
                class="border-b border-slate-100 bg-slate-50/50 px-6 py-4 dark:border-slate-800 dark:bg-slate-900/50"
            >
                <div class="flex flex-wrap items-center justify-between gap-4">
                    ...
                    @endphp
                    <span
                        class="rounded-full border px-4 py-1 text-xs font-black uppercase tracking-widest {{ $statusClasses[$item->status] ?? '' }}"
                    >
                        {{ $item->status }}
                    </span>
                </div>
            ...
            <div class="p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        >
                        <div
                            class="mt-2 rounded-xl bg-slate-50 p-4 text-sm leading-relaxed text-slate-700 dark:bg-slate-900 dark:text-slate-300"
                        >
                            {{ $item->keterangan }}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mt-2">
                            @if ($item->file_bukti)
                                ...
                                >
                                    <div
                                        class="flex h-10 w-10 items-center justify-center rounded-lg bg-red-50 text-red-600 group-hover:bg-red-600 group-hover:text-white transition-colors"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6">
                                    ...
                                    <div>
                                        ...
                                        >
                                            Klik untuk memperbesar gambar/PDF
                                        </div>
                                    </div>
                                </a>
                            @else
                                <div
                                    class="flex items-center gap-2 text-sm text-slate-400 italic"
                                >
            ...
            @if ($item->status === 'pending')
                <div
                    class="bg-slate-50 px-6 py-4 dark:bg-slate-900/30 border-t border-slate-100 dark:border-slate-800"
                >
                    <div class="flex flex-col gap-3 md:flex-row md:justify-end">
                        <form
                            method="POST"
                            action="{{ route('guru.surat-izin.reject', $item->surat_id) }}"
                            onsubmit="
                                return confirm(
                        ...
                        >
                            @csrf
                            <button
                                class="w-full rounded-xl border-2 border-rose-600 bg-white px-6 py-2.5 text-sm font-bold text-rose-600 transition-all hover:bg-rose-600 hover:text-white active:scale-95 dark:bg-transparent"
                            >
                                Tolak Pengajuan
                            </button>
                        </form>
                        <form
                            method="POST"
                            action="{{ route('guru.surat-izin.accept', $item->surat_id) }}"
                            class="w-full md:w-auto"
                        >
                            @csrf
                            <button
                                class="w-full rounded-xl bg-emerald-600 px-8 py-2.5 text-sm font-bold text-white shadow-lg shadow-emerald-600/20 transition-all hover:bg-emerald-700 active:scale-95"
                            >
                                Terima & Konfirmasi
                            </button>
                        </form>
            ...
            @else
                <div
                    class="bg-slate-50 px-6 py-4 dark:bg-slate-900/30 border-t border-slate-100 dark:border-slate-800 text-center"
                >
                    <p class="text-sm font-medium text-slate-500">Surat ini telah diproses pada status <span class="font-bold text-slate-700 dark:text-white underline">{{ Str::upper($item->status) }}</span>.</p>
                </div>
            @endif
        </div>
    </div>
