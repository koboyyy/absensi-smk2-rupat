    ...
    >
        <div>
            ...
            <div class="text-sm text-slate-500 dark:text-slate-400">
                Kelola surat izin dan sakit dari orang tua siswa.
            </div>
        </div>
        <div
            class="flex items-center gap-2 bg-slate-100 dark:bg-slate-900 p-1 rounded-2xl"
        >
            @foreach (['pending' => 'Pending', 'diterima' => 'Diterima', 'ditolak' => 'Ditolak'] as $k => $lbl)
                <a
                    href="{{ route('guru.surat-izin.index', ['status' => $k]) }}"
    ...
    >
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead
                    class="bg-slate-50 text-slate-600 dark:bg-slate-900/60 dark:text-slate-400 uppercase text-[11px] tracking-wider font-semibold"
                >
                    <tr>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse ($items as $item)
                        ...
                        >
                            <td
                                class="px-6 py-4 whitespace-nowrap font-medium text-slate-600 dark:text-slate-300"
                            >
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div
                                    class="text-[11px] text-slate-400 uppercase tracking-tighter"
                                >
                                    {{ $item->siswa?->nisn }}
                                </div>
                            </td>
                            ...
                            @endphp
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border {{ $currentClass }}"
                                >
                            ...
                            <td class="px-6 py-4 text-right">
                                <a
                                    href="{{ route('guru.surat-izin.show', $item->surat_id) }}"
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    <div
                                        class="rounded-full bg-slate-100 dark:bg-slate-900 p-3 mb-3"
                                    >
