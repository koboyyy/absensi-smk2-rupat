...
<script src="https://cdn.jsdelivr.net/npm/flatpickr"></script>
    ...
    <form method="POST" action="{{ route('admin.jadwal.update', $item->jadwal_id) }}"
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
                ...
                <input type="text" name="jam_mulai" id="jam_mulai" value="{{ old('jam_mulai', \Carbon\Carbon::parse($item->jam_mulai)->format('H:i')) }}"
            ...
            <div>
                ...
                <input type="text" name="jam_selesai" id="jam_selesai" value="{{ old('jam_selesai', \Carbon\Carbon::parse($item->jam_selesai)->format('H:i')) }}"
                       class="timepicker w-full rounded-xl border border-slate-200 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
                ...
                <select name="mapel_id" class="w-full rounded-xl border border-slate-200 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
                    @foreach($mapel as $m)
                        <option value="{{ $m->mapel_id }}" @selected(old('mapel_id', $item->mapel_id) == $m->mapel_id)>{{ $m->kode_mapel }} - {{ $m->nama_mapel }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="guru_id" class="w-full rounded-xl border border-slate-200 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
                    @foreach($guru as $g)
                        <option value="{{ $g->guru_id }}" @selected(old('guru_id', $item->guru_id) == $g->guru_id)>{{ $g->nama_guru }}</option>
                    @endforeach
                </select>
        ...
        <button class="rounded-xl bg-blue-600 px-4 py-3 font-semibold text-white hover:bg-blue-700">Simpan Perubahan</button>
    </form>
