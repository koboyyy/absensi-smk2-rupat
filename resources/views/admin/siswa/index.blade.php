...
@section ('content')
    <div class="mb-4 flex items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
            <a
                href="{{ route('admin.siswa.export.pdf') }}"
        ...
        <form method="GET" action="{{ route('admin.siswa.index') }}">
            <div class="grid grid-cols-1 gap-3 md:grid-cols-5">
                ...
                <a
                    ...
                    class="rounded-lg border border-slate-300 px-4 py-2 text-center text-sm hover:bg-slate-100 dark:border-slate-700 dark:hover:bg-slate-800"
                >
                    Reset
                </a>
            </div>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                    @foreach ($items as $index => $item)
                        <tr>
                            ...
                            {{-- Sesuaikan relasi jurusan --}}
                            <td class="px-4 py-3">
                                {{ $item->orangTua?->nama_ortu }}
                            </td>
                            <td class="px-4 py-3 truncate max-w-[150px]">
                                {{ $item->alamat }}
                            </td>
                            {{-- Sesuaikan field alamat --}}
                            <td class="px-4 py-3">
                                @if ($item->foto)
                                    <img
                                        src="{{ asset('storage/siswa/' . $item->foto) }}"
                            ...
                            <td class="px-4 py-3">
                                <div class="flex justify-end gap-2">
                                    <a
                                        href="{{ route('admin.siswa.edit', $item->siswa_id) }}"
                                        class="rounded-lg px-2 py-1 text-xs font-semibold hover:bg-slate-100 dark:hover:bg-slate-900"
                                        title="Edit"
                                    >
                                    ...
                                    <form
                                        method="POST"
                                        action="{{ route('admin.siswa.destroy', $item->siswa_id) }}"
                                        onsubmit="
                                    ...
                                    >
                                        ...
                                        @method ('DELETE')
                                        <button
                                            class="rounded-lg px-2 py-1 text-xs font-semibold text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"
                                            title="Hapus"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
