...
@section('content')
    <div class="mb-4 flex items-center justify-between">
        <div class="text-xl font-bold">Edit Siswa</div>
    ...
    <form method="POST" action="{{ route('admin.siswa.update', $item->siswa_id) }}"
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
                ...
                <input name="nis" value="{{ old('nis', $item->nis) }}" class="w-full rounded-xl border border-slate-200 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
            ...
            <div>
                ...
                <input name="nama_siswa" value="{{ old('nama_siswa', $item->nama_siswa) }}" class="w-full rounded-xl border border-slate-200 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
                ...
                <select name="kelas_id" class="w-full rounded-xl border border-slate-200 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
                    @foreach($kelas as $k)
                        <option value="{{ $k->kelas_id }}" @selected(old('kelas_id', $item->kelas_id)==$k->kelas_id)>{{ $k->nama_kelas }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="ortu_id" class="w-full rounded-xl border border-slate-200 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
                    <option value="">-</option>
                    @foreach($ortu as $o)
                        <option value="{{ $o->ortu_id }}" @selected(old('ortu_id', $item->ortu_id)==$o->ortu_id)>{{ $o->nama_ortu }}</option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <input name="alamat" value="{{ old('alamat', $item->alamat) }}" class="w-full rounded-xl border border-slate-200 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
        ...
        <div>
            ...
            <input name="foto" value="{{ old('foto', $item->foto) }}" class="w-full rounded-xl border border-slate-200 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
