...
@section('content')
    <div class="mb-4 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
            ...
            <div class="text-sm text-slate-500 dark:text-slate-400">Validasi absensi siswa di kelas binaan.</div>
        </div>
        <form method="GET" action="{{ route('wali.validasi.index') }}" class="flex items-center gap-2">
    ...
    <form method="POST" action="{{ route('wali.validasi.batch') }}"
            ...
            <table class="w-full text-left text-sm">
                ...
                @forelse($items as $it)
                    <tr>
                        <td class="px-4 py-3">
                            @if(!$it->wali_validasi)
                                <input type="checkbox" name="absensi_id[]" value="{{ $it->absensi_id }}">
                            @else
                                <span class="text-xs text-slate-500">-</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 font-medium">{{ $it->siswa?->nama_siswa }}</td>
                        <td class="px-4 py-3">{{ $it->jadwal?->mapel?->nama_mapel }}</td>
                        <td class="px-4 py-3">{{ $it->status }}</td>
                        <td class="px-4 py-3">
                            <span class="rounded-full px-2 py-1 text-xs font-semibold {{ $it->wali_validasi ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-700' }}">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-6 text-center text-slate-500 dark:text-slate-400">Tidak ada data.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
