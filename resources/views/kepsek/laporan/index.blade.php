...
@section('content')
    <div class="mb-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
            ...
            <div class="text-sm text-slate-500 dark:text-slate-400">Pantau rekap keseluruhan per jurusan & kelas.</div>
        </div>
        <a href="{{ route('kepsek.laporan.export.pdf', request()->query()) }}"
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
        @foreach (['H' => 'Hadir', 'S' => 'Sakit', 'I' => 'Izin', 'A' => 'Alfa'] as $k => $label)
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                ...
                <div class="mt-2 text-3xl font-extrabold text-blue-700 dark:text-blue-400">{{ $summary[$k] }}</div>
            </div>
        @endforeach
    </div>
        ...
        <div class="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800">
            <table class="w-full text-left text-sm">
                ...
                @foreach($perJurusan as $r)
                    <tr>
                        <td class="px-4 py-3 font-medium">{{ $r->jurusan }}</td>
